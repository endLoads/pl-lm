(function () {
  ...
  // ================= РАСКРАСКА МЕТОК (используется темой) =================
        ...
        .then(function (text) {
          ...
          try {
            ...
            if (json && json.films && json.films.length) {
              ...
              film = json.items[0];
            } else if (json && (json.ratingKinopoisk || json.ratingImdb || json.rating)) {
              film = json;
            }
  ...
  function injectBorderlessDarkTheme(enabled) {
    try {
      var id = "drx-borderless-dark-theme";
      ...
      var css = [
        ...
        ".card, .card__view, .card__body {",
        "  border-radius: 16px !important;",
        "  box-shadow: 0 12px 40px rgba(0,0,0,0.9) !important;",
        "  border: none !important;",
        "}",
        ".card__title, .card__rating, .card__votes {",
  ...
  function setBorderlessDarkThemeEnabled(enabled) {
    ...
    injectBorderlessDarkTheme(enabled);
  }
      ...
      Lampa.SettingsApi.addComponent({
        ...
        onRender: function (body) {
          ...
          html.innerHTML = [
            ...
            "<div class=\"settings-param__name\">Рейтинг КиноПоиска</div>",
            "<div class=\"settings-param__descr\">Включает запрос к КиноПоиску (нужен API‑ключ).</div>",
            "<div class=\"settings-param__value\">ключ хранится в SuperMenuConfig.RATINGS.kpApiKey</div>",
            "</div>",
            "<div class=\"settings-param\">",
            "<div class=\"settings-param__name\">Цветные метки</div>",
            "<div class=\"settings-param__descr\">Раскраска типа и качества карточек.</div>",
            "</div>",
            "<div class=\"settings-param\">",
            "<div class=\"settings-param__name\">Тема: тёмная без рамок</div>",
            "<div class=\"settings-param__descr\">Переключается из темы или отсюда через API.</div>",
            "</div>"
          ].join("");
  ...
  function registerTopBarButton() {
    try {
      ...
      Lampa.Panel.add({
        ...
        onClick: function () {
          try {
            if (Lampa.Noty) Lampa.Noty.show("DrxSuperMenu: меню выхода пока не реализовано");
          } catch (e) {
  ...
  function start() {
    try {
      ...
      injectBorderlessDarkTheme(SuperMenuConfig.FEATURES.borderless_dark_theme);
      exportApi();
