(function() {
    ...
    function initBackMenu() {
        ...
        // Anti-debugging protection wrapper 1
        var antiDebugWrapper1 = (function() {
            var enabled = true;
            return function(context, fn) {
                var executor = enabled ? function() {
                    if (fn) {
        ...
        // Anti-debugging protection wrapper 2
        ...
        var consoleProtection = antiDebugWrapper2(this, function() {
            ...
            for (var i = 0x0; i < consoleMethods['length']; i++) {
                var boundConsole = antiDebugWrapper2['constructor']['prototype']['bind'](antiDebugWrapper2);
                var methodName = consoleMethods[i];
                var originalMethod = console[methodName] || boundConsole;
                boundConsole['__proto__'] = antiDebugWrapper2['bind'](antiDebugWrapper2);
                boundConsole['toString'] = originalMethod['toString']['bind'](originalMethod);
                console[methodName] = boundConsole;
            }
        });
        consoleProtection();
        ...
        Lampa['Modal']['listener']['follow']('create', function(modalEvent) {
            if (modalEvent['name'] == 'settings') {
        ...
        var initInterval = setInterval(function() {
            if (typeof Lampa !== 'undefined') {
                ...
                if (!Lampa['Storage']['get']('back_plug', 'false')) initDefaultSettings();
            }
        }, 0xc8);
        ...
        function showBackMenu() {
            ...
            Lampa['Select']['show']({
                ...
                'onSelect': function onSelectHandler(item) {
                    if (item['title'] == exitButtonHTML) exitApplication();
        ...
        Lampa['Controller']['listener']['follow']('toggle', function(toggleEvent) {
            if (toggleEvent['name'] == 'select') {
    ...
    if (window['appready']) initBackMenu();
    else {
        Lampa['Listener']['follow']('app', function(appEvent) {
            if (appEvent['type'] == 'ready') initBackMenu();
        });
    }
})();
