    ...
    function initPlugin() {
        ...
        Lampa.Settings.addComponent({
            ...
            onRender: function(element) {
                element.on('hover:enter', function() {
                    Lampa.Settings.create('BackMenu');
                    Lampa.Controller.toggle('back_menu');
                    Lampa.Controller.listener.follow('select', function(e) {
                        if (e.name == 'select' && $('.selectbox__title').text() == Lampa.Lang.translate('back_menu')) {
                            showExitMenu();
                            setTimeout(function() {
                                $('.selectbox').hide();
        ...
        var checkInitInterval = setInterval(function() {
            if (typeof Lampa !== 'undefined') {
                ...
                if (!Lampa.Storage.field('back_menu_set', 'false')) {
                    setDefaultSettings();
                }
            }
        }, 200);
        ...
        function setDefaultSettings() {
            Lampa.Storage.set('back_menu_set', true);
        ...
        function showSpeedTest() {
            var iframe = $('<div style="text-align:right;"><div style="min-height:360px;"><iframe id="speedtest-iframe" width="100%" height="100%" frameborder="0"></iframe></div></div>');
        ...
        function showServerInput() {
            Lampa.Input.edit({
                ...
                if (serverName !== '') {
                    window.location.assign(baseUrl + serverName);
                } else {
                    showExitMenu();
                }
            });
        ...
        function exitApplication() {
            ...
            if (Lampa.Platform.is('webos')) window.history.back();
            if (Lampa.Platform.is('browser')) window.close();
            if (Lampa.Platform.is('nw')) nw.App.quit();
        }

        function showExitMenu() {
            var currentControllerName = Lampa.Controller.listener.follow('select', function() {}).name;
            var menuItems = [];

            if (localStorage.getItem('exit') !== '1') menuItems.push({ title: 'Выход ' });
            if (localStorage.getItem('reboot') !== '1') menuItems.push({ title: 'Перезагрузить' });
            if (localStorage.getItem('switch_server') !== '1') menuItems.push({ title: 'Сменить сервер' });
            if (localStorage.getItem('clear_cache') !== '1') menuItems.push({ title: 'Очистить кэш' });
            if (localStorage.getItem('youtube') !== '1') menuItems.push({ title: youtubeMenuItem });
            if (localStorage.getItem('rutube') !== '1') menuItems.push({ title: rutubeMenuItem });
            if (localStorage.getItem('drm_play') !== '1') menuItems.push({ title: drmPlayMenuItem });
            if (localStorage.getItem('twitch') !== '1') menuItems.push({ title: twitchMenuItem });
            if (localStorage.getItem('fork_player') !== '1') menuItems.push({ title: forkPlayerMenuItem });
            if (localStorage.getItem('speedtest') !== '1') menuItems.push({ title: speedTestMenuItem });
            ...
            Lampa.Noty.show({
                ...
                onSelect: function(item) {
                    if (item.title == 'Выход ') exitApplication();
                    if (item.title == 'Перезагрузить') location.reload();
                    if (item.title == 'Сменить сервер') showServerInput();
                    if (item.title == 'Очистить кэш') clearCache();
                    if (item.title == youtubeMenuItem) window.location.assign('https://youtube.com/tv');
                    if (item.title == rutubeMenuItem) window.location.assign('https://rutube.ru/tv-release/rutube.server-22.0.0/webos/');
                    if (item.title == drmPlayMenuItem) window.location.assign('https://ott.drm-play.com');
                    if (item.title == twitchMenuItem) window.location.assign('https://webos.tv.twitch.tv');
                    if (item.title == forkPlayerMenuItem) window.location.assign('http://browser.appfxml.com');
                    if (item.title == speedTestMenuItem) showSpeedTest();
                }
            });
        }
