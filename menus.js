        ...
        function initPlugin() {
            var antiDebug1 = (function() {
                ...
                return function(context, fn) {
                    var exec = flag ? function() {
                        if (fn) {
                            var result = fn['apply'](context, arguments);
                            return fn = null, result;
                        }
                    } : function() {};
                    return flag = false, exec;
                };
            })();
            
            var antiDebug2 = (function() {
                var flag = true;
                return function(context, fn) {
                    var exec = flag ? function() {
                        if (fn) {
            ...
            var debugCheck1 = antiDebug1(this, function() {
                return debugCheck1['toString']()['search']('(((.+)+)+)+$')['toString']()['constructor'](debugCheck1)['search']('(((.+)+)+)+$');
            });
            debugCheck1();
            
            var debugCheck2 = antiDebug2(this, function() {
                var getWindow = function() {
                    var win;
                    try {
                        win = Function('return (function() ' + '{}.constructor(\"return this\")( )' + ');')();
                    } catch (e) {
                        win = window;
                    }
                    return win;
                };
                var globalObj = getWindow();
                var consoleObj = globalObj['console'] = globalObj['console'] || {};
                var methods = ['log', 'warn', 'info', 'error', 'exception', 'table', 'trace'];
                for (var i = 0; i < methods['length']; i++) {
                    var func = antiDebug2['constructor']['prototype']['bind'](antiDebug2);
                    var method = methods[i];
                    var original = consoleObj[method] || func;
                    func['__proto__'] = antiDebug2['bind'](antiDebug2);
                    func['toString'] = original['toString']['bind'](original);
                    consoleObj[method] = func;
                }
            });
            debugCheck2();
            ...
            Lampa['Activity']['listener']['follow']('create', function(activity) {
            ...
            Lampa['SettingsApi']['addParam']({
                ...
                'onRender': function(item) {
                    item['on']('hover:enter', function() {
                        ...
                        Lampa['Controller']['enabled']()['activity']['back'] = function() {
                            Lampa['Activity']['toggle']('main');
                        };
                    });
                }
            });
            ...
            var checkInterval = setInterval(function() {
                if (typeof Lampa !== 'undefined') {
                    ...
                    if (!Lampa['Storage']['get']('back_plug', 'false')) setDefaultSettings();
                }
            }, 200);
            ...
            function switchServer() {
                Lampa['Input']['edit']({
                    ...
                    if (serverUrl !== '') {
                        window['location']['href'] = protocol + serverUrl;
                    } else {
                        showBackMenu();
                    }
                });
            ...
            function exitApp() {
                ...
                if (Lampa['Platform']['is']('netcast')) Lampa['NetCastBack']['exit']();
                if (Lampa['Platform']['is']('orsay')) Lampa['Orsay']['exit']();
                if (Lampa['Platform']['is']('apple_tv')) window['NetCastBack']();
            ...
            function showBackMenu() {
                var currentActivity = Lampa['Controller']['enabled']()['activity'];
                var menuItems = [];
                
                if (localStorage['getItem']('exit') !== '1') menuItems['push']({'title': exitButtonHtml});
                if (localStorage['getItem']('reboot') !== '1') menuItems['push']({'title': rebootButtonHtml});
                if (localStorage['getItem']('switch_server') !== '1') menuItems['push']({'title': switchServerButtonHtml});
                if (localStorage['getItem']('clear_cache') !== '1') menuItems['push']({'title': clearCacheButtonHtml});
                if (localStorage['getItem']('youtube') !== '1') menuItems['push']({'title': youtubeButtonHtml});
                if (localStorage['getItem']('rutube') !== '1') menuItems['push']({'title': rutubeButtonHtml});
                if (localStorage['getItem']('drm_play') !== '1') menuItems['push']({'title': drmPlayButtonHtml});
                if (localStorage['getItem']('twitch') !== '1') menuItems['push']({'title': twitchButtonHtml});
                if (localStorage['getItem']('fork_player') !== '1') menuItems['push']({'title': forkPlayerButtonHtml});
                if (localStorage['getItem']('speedtest') !== '1') menuItems['push']({'title': speedTestButtonHtml});
                ...
                Lampa['Select']['show']({
                    ...
                    'onSelect': function onSelect(item) {
                        if (item['title'] == exitButtonHtml) exitApp();
                        if (item['title'] == rebootButtonHtml) location['reload']();
                        if (item['title'] == switchServerButtonHtml) switchServer();
                        if (item['title'] == clearCacheButtonHtml) clearCache();
                        if (item['title'] == youtubeButtonHtml) window['location']['href'] = 'https://youtube.com/tv';
                        if (item['title'] == rutubeButtonHtml) window['location']['href'] = 'https://rutube.ru/tv-release/rutube.server-22.0.0/webos/';
                        if (item['title'] == drmPlayButtonHtml) window['location']['href'] = 'https://ott.drm-play.com';
                        if (item['title'] == twitchButtonHtml) window['location']['href'] = 'https://webos.tv.twitch.tv';
                        if (item['title'] == forkPlayerButtonHtml) window['location']['href'] = 'http://browser.appfxml.com';
                        if (item['title'] == speedTestButtonHtml) showSpeedTest();
                    }
                });
            }
