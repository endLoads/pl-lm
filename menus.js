(function() {
    ...
    function initPlugin() {
        // Убираем проверку доступа к платформе
        ...
        addMenuItemSetting('exit', 'Закрыть приложение');
        addMenuItemSetting('reboot', 'Перезагрузить');
        addMenuItemSetting('switch_server', 'Сменить сервер');
        addMenuItemSetting('clear_cache', 'Очистить кэш');
        addMenuItemSetting('youtube', 'YouTube');
        addMenuItemSetting('rutube', 'RuTube');
        addMenuItemSetting('drm_play', 'DRM Play');
        addMenuItemSetting('twitch', 'Twitch');
        addMenuItemSetting('fork_player', 'ForkPlayer');
        addMenuItemSetting('speedtest', 'Speed Test');

        // Устанавливаем настройки по умолчанию
        setDefaultSettings();

        // Функция для показа меню выхода
        function showExitMenu() {
            Lampa.Noty.show({
                ...
                onSelect: function(item) {
                    handleMenuItemSelection(item.title);
                }
            });
        }

        // Функция для получения пунктов меню в зависимости от настроек
        function getMenuItems() {
            const items = [];
            
            if (getSettingValue('exit') !== '1') items.push({ title: 'Выход ' });
            if (getSettingValue('reboot') !== '1') items.push({ title: 'Перезагрузить' });
            if (getSettingValue('switch_server') !== '1') items.push({ title: 'Сменить сервер' });
            if (getSettingValue('clear_cache') !== '1') items.push({ title: 'Очистить кэш' });
            if (getSettingValue('youtube') !== '1') items.push({ title: MENU_ITEMS.youtube });
            if (getSettingValue('rutube') !== '1') items.push({ title: MENU_ITEMS.rutube });
            if (getSettingValue('drm_play') !== '1') items.push({ title: MENU_ITEMS.drm_play });
            if (getSettingValue('twitch') !== '1') items.push({ title: MENU_ITEMS.twitch });
            if (getSettingValue('fork_player') !== '1') items.push({ title: MENU_ITEMS.fork_player });
            if (getSettingValue('speedtest') !== '1') items.push({ title: MENU_ITEMS.speedtest });
            
            return items;
        }

        // Функция для обработки выбора пункта меню
        function handleMenuItemSelection(title) {
            if (title === 'Выход ') exitApplication();
            else if (title === 'Перезагрузить') location.reload();
            else if (title === 'Сменить сервер') showServerInput();
            else if (title === 'Очистить кэш') clearCache();
            else if (title === MENU_ITEMS.youtube) openYouTube();
            else if (title === MENU_ITEMS.rutube) openRuTube();
            else if (title === MENU_ITEMS.drm_play) openDRMPlay();
            else if (title === MENU_ITEMS.twitch) openTwitch();
            else if (title === MENU_ITEMS.fork_player) openForkPlayer();
            else if (title === MENU_ITEMS.speedtest) openSpeedTest();
        }
        ...
        function showServerInput() {
            const baseUrl = location.protocol === 'https:' ? 'https://' : 'http://';
            ...
            Lampa.Input.edit({
                ...
                if (serverName !== '') {
                    window.location.href = baseUrl + serverName;
                }
            });
        }
        ...
        function openForkPlayer() {
            window.location.href = 'http://browser.appfxml.com';
        }

        function openSpeedTest() {
            const iframe = $('<div style="text-align:right;"><div style="min-height:360px;"><iframe id="speedtest-iframe" width="100%" height="100%" frameborder="0"></iframe></div></div>');
        ...
        function addMenuItemSetting(name, title) {
        ...
        function setDefaultSettings() {
            // Убираем проверку установки настроек
            // if (!Lampa.Storage.field('back_menu_set')) {
